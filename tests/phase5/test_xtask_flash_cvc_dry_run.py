...
STM32_Programmer_CLI. The test asserts:
  ...
  3. The printed command contains `sn=001A00363235510B37333439`
...
# Serial from tools/bench/hardware-map.toml [[stlink]] logical_ecu = "cvc"
# As of 2026-04-15 (ff79c92). If the map rotates, update this expectation.
CVC_STLINK_SERIAL = "001A00363235510B37333439"
...
def test_flash_cvc_dry_run_prints_programmer_cli_invocation() -> None:
    if not XTASK_MANIFEST.exists():
        pytest.fail("xtask crate not scaffolded yet (D2 red)")

    result = _run_xtask("flash-cvc", "--dry-run")
...
def test_flash_cvc_defaults_to_dry_run() -> None:
...
def test_flash_cvc_reads_serial_from_hardware_map() -> None:
